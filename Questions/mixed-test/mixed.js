...
function startQuiz() {
  ...
  loadQuestions()
    .then((loadedQuestions) => {
      questions = loadedQuestions.slice(0, 10); // Nur die ersten 10 Fragen verwenden
...
// Laden der Fragen aus den JSON-Dateien in den Ordnern
function loadQuestions() {
  const folders = [
    ...
    '../history-test',
    '../movie-test',
    '../science_and_technology-test',
  ];

  const promises = folders.map((folder) =>
    fetch(`${folder}/questions.json`).then((response) => response.json())
  );

  return Promise.all(promises).then((data) => {
    const combinedQuestions = data.flatMap((questions) =>
      shuffleQuestions(questions).slice(0, 2)
    );
...
function showQuestion() {
  ...
  // Die Antwortmöglichkeiten bleiben in der ursprünglichen Reihenfolge
...
// Überprüft die ausgewählte Antwort und zeigt die nächste Frage an
...
function startTimer() {
  ...
  timer = setInterval(() => {
    ...
    if (width >= 100) {
      clearInterval(timer);
      if (!quizEnded) { 
        // Timer stoppen, wenn das Quiz noch nicht beendet ist
        currentQuestionIndex++;
        if (currentQuestionIndex < questions.length) {
          resetTimer(); // Timer zurücksetzen
          showQuestion();
